...
function applyHubNavigation(){
  ...
  if(file==='NUR2460Flashcards.html'){
    ...
    const context=[exam||'All exams',validGroup?group:null,mode==='due'?'Due-only review':mode==='review'||mode==='study'||validGroup?'Review':'Browse'].filter(Boolean).join(' · ');
    ...
    if(mode==='due'&&!deck.length){const empty=document.createElement('p');empty.textContent='No active cards are due in this scope. Nothing was added to your review history.';banner.appendChild(empty)}
  }
